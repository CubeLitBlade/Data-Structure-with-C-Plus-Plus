...
namespace SeqList_AC1
{
     ...
     * \note 此方法由 @KepWalk 提供。
     */
    void QuickPrefixNegative(const DataStructure::SeqList<int>& target)
    {
        DataStructure::SeqListIterator<int> leftIterator = target.begin(), rightIterator = ++target.begin();
        for (auto temp = target.begin(); temp != target.end(); ++temp)
        {
            if (rightIterator == target.end()) break;
            if (*temp >= 0)
            {
                leftIterator = temp;
                rightIterator = (leftIterator + 1 > rightIterator) ? leftIterator + 1 : rightIterator;

                while (rightIterator != target.end())
                {
                    if (*rightIterator < 0)
                    {
                        // 交换元素，并且退出遍历操作。
