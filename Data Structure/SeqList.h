...
namespace DataStructure
{
	...
	public:
		...
		size_t find(const T& target) const override;
	...
	template <typename T>
	inline bool SeqList<T>::empty() const
	{
	...
	// 获取顺序表的长度（即表内的元素个数）。
	template <typename T>
	inline size_t SeqList<T>::length() const
	{
	...
	// 获取顺序表的容量（即顺序表的最大长度）。
	template <typename T>
	inline size_t SeqList<T>::size() const
	{
		return this->m_size;
	}
	...
	template <typename T>
	size_t SeqList<T>::find(const T& target) const
	{
	...
	template <typename T>
	inline const T& SeqList<T>::get(size_t index) const
	{
