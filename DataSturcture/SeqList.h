...
namespace DataStructure
{
	...
	template <typename T>
	const T& SeqList<T>::get(size_t index)
	{
	...
	// 从相应下标处插入元素。成功返回 true ，失败返回 false 。若下标超出范围，抛出 std::invalid_argument 异常。
	template<typename T>
	bool SeqList<T>::insert(size_t index, const T& target)
	{
		...
		{
			throw std::invalid_argument(DataStructureException::BAD_INDEX);
		}
		else
		{
	...
	// 移除特定下标处的元素。成功返回 true ，失败返回 false 。若下标超出范围，抛出 std::invalid_argument 异常。
	template<typename T>
